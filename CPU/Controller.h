...
public:
    ...
    void setControllBits(uint16_t b);
    bool *controllbits[c];
...
#define J 0b0100000000000000
    ...
    uint16_t instructionset[16][8]={
            {MI|CO,RO|II|CE,0,0,0,0,0,0},// 0000 - NOP
            {MI|CO,RO|II|CE,IO|MI,RO|AI,0,0,0,0},// 0001 - LDA
            {MI|CO,RO|II|CE,IO|MI,RO|BI,EO|AI,0,0,0},// 0010 - ADD
            {MI|CO,RO|II|CE,IO|MI,EO|AI|SU,0,0,0,0},// 0011 - SUB
...
template<typename T, int c>
void Controller<T, c>::update() {
   // setControllBits(instructionset[(instructionRegister->value>>4)][stepcounter]);

}

template<typename T, int c>
void Controller<T, c>::defineControllBits(bool *b[c]) {
    for (int i = 0; i < c; i++) {
...
template<typename T, int c>
void Controller<T, c>::setControllBits(uint16_t b) {
